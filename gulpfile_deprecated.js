...
gulp.task('serve', ['move-html', 'sass', 'concat-js'], function serve() {
  browserSync.init({
    server: './dist'
  })
...
gulp.task('browser-reload', ['move-html', 'sass', 'concat-js'], function browserReload() {
  browserSync.stream()
})

//  將不同的 js 檔案合併在同一支當中
gulp.task('concat-js', function concatJS() {
  return gulp.src([
...
//  clean-css
gulp.task('clean-css', ['sass'], function cleanCSS() {
